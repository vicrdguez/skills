...
4. Review `git diff 0000000000000000000000000000000000000004...HEAD` as a repeat review: verify every still-active finding against the final state, read the diff for regressions, integration effects and false claims in the updated report, and scan the whole change only for the critical class. Assign a new `W<n>` only for a defect the rework introduced or a critical discovery. Apply the method and criteria below.
