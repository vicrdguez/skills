...
`skl skill --resource reference/ledger-submission.md --input result_directory='/tmp/skl-implement-result' --input procedure=resumed implement`
...
`skl implement inspect --repo '/work/widgets' --remote 'origin' --item 'widget-dashboard/foundation' --claim '0000000000000000000000000000000000000001' --result-directory '/tmp/skl-implement-result' --target <observed-target-sha>` before Audit, substituting the exact newly integrated
