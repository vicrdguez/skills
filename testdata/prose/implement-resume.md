# Implement `widget-dashboard/foundation` (resumed)
...
`skl skill --resource ledger-submission.md --input result_directory='/tmp/skl-implement-result' --input procedure=resumed implement`
...
1. Run the inspection command with `<observed-target-sha>` as its `--target`: `skl implement inspect --repo '/work/widgets' --remote 'origin' --item 'widget-dashboard/foundation' --claim '0000000000000000000000000000000000000001' --result-directory '/tmp/skl-implement-result' --target <observed-target-sha>`. Its output is the input-inspection result.
