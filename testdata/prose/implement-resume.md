...
This Claim already has work. Establish from the branch and the facts above what remains, keep the existing commits and recorded references, and deliver the rest of the Contract.
...
Retrieve the report template with `skl skill --resource ledger-submission.md --input result_directory='/tmp/skl-implement-result' --input procedure=resumed implement`, and write the documents it describes. Commit all source changes.
