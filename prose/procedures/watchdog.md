...
Prepare the worktree with `{{.PrepareCommand}}`, then inspect it with `{{.InspectCommand}}`. Work only in `{{.Worktree}}`. Continue this Claim with `{{.ResumeCommand}}`; release it with `{{.ReleaseCommand}}` only to abandon the review.
...
4. {{if eq .ReviewScope "incremental"}}Review {{template "watchdog-incremental" .}}.{{else if or (eq .ReviewScope "full") (not .PreviousReviewed)}}Review {{template "watchdog-full" .}}.{{else}}When inspection reports `incremental`, review {{template "watchdog-incremental" .}}; otherwise review {{template "watchdog-full" .}}.{{end}} Apply the method and criteria below. Review against the recorded Integration Target, this round's cutoff: the merge and its conflict resolutions count, unrelated inherited target code does not.
...
You edit no functional code. Write the report as `{{.ResultResourceCommand}}` instructs, then submit with `{{.SubmitCommand}}`:
