...
{{if eq .Procedure "resumed"}}This Claim already has work. Establish from the branch and the facts above what remains, keep the existing commits and recorded references, and deliver the rest of the Contract.
...
Finish the unblocked work and write the blocking report from the step 5 template. Then pause with `{{.PauseCommand}}`, using the preparation target as `<observed-target-sha>` if you have not merged yet. Leave a conflicted merge in place.
