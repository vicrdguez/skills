...
// never sent for a superseded selection. It returns the attachment to report
// and this invocation's outcome note.
func presentIssue(ctx context.Context, forge Forge, repository string, selected *ForgeAttachment, title, body string, recorded func() (*ForgeAttachment, error)) (*ForgeAttachment, *PublicationNote) {
	...
	if err != nil {
		return selected, &PublicationNote{Status: IssueFailed, Detail: "the current local record is unreadable: " + err.Error()}
	}
	if !sameAttachment(current, selected) {
		return current, &PublicationNote{Status: IssueSuperseded, Detail: "the local record changed during this invocation; publish again to present the current view"}
	}
	if selected != nil {
		if err := forge.UpdateIssue(ctx, selected.Number, title, body); err != nil {
...
// a failure is reported, never saved as a retry plan.
func groupChildren(ctx context.Context, store *Store, outcome *Acceptance, forge Forge) {
	...
	for index := range outcome.Slices {
		...
		if child.Issue == nil || slices.Contains(linked, child.Issue.Number) {
			continue
		}
		parent, err := store.recordedParent(outcome.Project, outcome.Proposal)
		if err == nil && parent != nil && !sameAttachment(parent, outcome.ParentIssue) {
			child.GroupingStatus = &PublicationNote{Status: IssueSuperseded, Detail: "the recorded parent issue changed during this invocation; publish again to group under it"}
			continue
		}
...
func (s *Store) recordPublication(outcome *Acceptance, pushed bool) (string, error) {
	...
	err := s.withMutation(func() error {
		...
		case meta.ParentIssue != nil && outcome.ParentIssue != nil && !sameAttachment(meta.ParentIssue, outcome.ParentIssue):
			outcome.ParentNote = conflictNote(outcome.ParentIssue, meta.ParentIssue)
		...
		for index := range outcome.Slices {
			...
			switch {
			case state.Issue != nil && acceptance.Issue != nil && !sameAttachment(state.Issue, acceptance.Issue):
				acceptance.IssueStatus = conflictNote(acceptance.Issue, state.Issue)
				acceptance.Issue = state.Issue
			case state.Issue != nil:
				acceptance.Issue = state.Issue
...
// conflictNote reports an attachment this invocation established while a
// different one was recorded concurrently; the recorded one is retained.
func conflictNote(established, recorded *ForgeAttachment) *PublicationNote {
	return &PublicationNote{
		Status: IssueConflict,
		Detail: fmt.Sprintf("this invocation established %s#%d, but %s#%d is already recorded and was retained; resolve the extra forge object with human direction", established.Repository, established.Number, recorded.Repository, recorded.Number),
	}
}
