...
func PublishDelivery(ctx context.Context, s *Store, repository github.RepositoryID, root, remote string, result *DeliveryResult, publicBody *string, forge DeliveryForge) {
	...
	proposal, slice, _ := strings.Cut(result.Item, "/")
	leases, err := s.publicationLeases(publicationLeaseName(repository.Owner+"/"+repository.Name, proposal, slice, publicationKindPull))
	if err != nil {
