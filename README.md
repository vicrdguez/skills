...
| Implement | `implement` | One claimed slice delivered against its accepted contract, verified at suitable seams, reviewed via `audit`, PR labeled `review` |
...
The rest are reference skills the stages pull in rather than stages of their own: `design` (deep modules and seams), `domain` (glossary, ADRs), `testing` (contract-grounded behavioral and regression evidence), `audit` (the two-axis review engine). `writing-for-agents` is a standalone reference for authoring skills and agent-facing docs.
...
Run `skl implement next` for one claimed Work Item and its complete Execution Skill, or `skl implement resume --item <number>` for interrupted work. Every Implement command defaults to Markdown and takes `--format json` when a machine-readable outcome is wanted; JSON is an equivalent transport of the same operation, never a second operation or an alternative authority for success. Inside its conventional worktree, `skl implement resume` resolves the Claim by location. Selection reads only open queue-labelled records: it exhausts eligible `rework` Submissions before `ready` issues, orders each queue by its own record age and stable numeric identity, skips claimed and blocked candidates before fetching their discussions, and resolves one explicit owning issue per Submission rather than matching titles or branches. `next` returns the complete Execution Skill for the selected Work Item, with the repository, branch, conventional worktree, selected remote, private result location, and concrete fetch/create/push/inspect/resume/submit/pause commands already bound; `start` is its alias, and `resume` continues a fixed identity instead of selecting other work. Startup reads no project objects and prepares no Git: the worker fetches and prepares the worktree, then resolves the Artifact Baseline and Completion with `skl implement inspect`, which returns a narrow continuation for the observed ledger progress and integrity rather than a second full skill. Pass `--capability claude-agents`, `--capability pi-subagents`, or `--capability sequential` to bind the helper recipe this invocation can actually run for optional implementation/testing delegation and mandatory Audit dispatch; omitting it keeps a bounded runtime choice, because an adapter or harness name alone proves nothing. An unsupported `--format` or `--capability` value is rejected before any backend call, selection, Claim, publication, or result-directory creation. Markerless existing work may supply full `--artifact-baseline` and `--artifact-completion` SHAs on relevant Implement and Watchdog commands; preserve those exact flags through every command for that invocation and Work Item.
...
The worker preserves existing branch progress, implements the complete accepted behavioral and architectural contract, chooses suitable existing verification boundaries and construction order, and runs focused checks. Tests may be grouped, reused, strengthened, consolidated, or removed while required behavioral and failure-mode protection remains covered; explicit frozen obligations still bind. Immediately before each submission Audit, the worker fetches the selected remote's current `main`, records its full SHA, merges that exact snapshot into the work branch, resolves conflicts, and verifies the integrated state. The observed SHA is the round's cutoff; preparation-time state does not satisfy the step, while later target movement alone does not restart it. Audit uses the integrated comparison, runs the Full Gate and both independent review axes against a provisional endpoint, which may still have unfinished boxes and must not be reported ready for review. After dispositions, functional edits receive affected checks and a Full Gate covering the final state. The worker then ticks every automated box, commits the still-present ledger with `[completion] <slug>`, removes it in a later commit, and pushes. `skl` runs none of those project checks or Git mutations. Any later integration with target movement, conflict resolution, and final merge of the reviewed Submission into `main` remain human-owned after review.
...
Startup uses selected metadata even when local project objects exist. A retained Review Checkpoint supplies the completed count and previous revision without claiming a verified comparison; inspection uses an available ancestral prior revision, including the unchanged head, or gives a full PR comparison with a fallback reason. A lost checkpoint does not erase supplied finding IDs or human dispositions. Workers do not locate, parse, or edit the `.watchdog` checkpoint. Watchdog checks the integrated target SHA recorded in Submission Verification against the fixed reviewed head and reviews merge or conflict-resolution effects, without treating unrelated target additions as scope creep or chasing later target movement beyond the recorded cutoff.
