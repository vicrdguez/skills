...
Implementation and Watchdog handoffs keep their destination protected while exact evidence is published and source projections are cleaned, then remove destination `wip` last. Retries use current Git, artifacts, visible feedback, supplied Result Documents, and retained review pins rather than transition journals or resume cursors; ambiguous Claims stop for inspection. A cleanup warning after verified publication is local cleanup only and does not require resubmission.
