...
- Agent Workers retain repository-content edits and ordinary Git operations such as commits, pushes, and implementation-ledger retirement. `skl` reads Git to enforce workflow preconditions and owns backend state transitions; it does not wrap Git merely to reduce tool calls. Integration with `main`, conflict resolution, and merge belong to the human Merge Authority. Deterministically identified workflow worktree and branch cleanup is the narrow exception: `skl` performs it as part of the transition that makes the cleanup valid.
...
## Planned correction: Simplify work startup and review state

The candidate-first correction changes earlier compatibility, synchronization, and bounce-counting policies while preserving agent judgment, independent Watchdog Review, and human-only final merging. The delivered integration-policy slice removes implementation target pins, uses `main` as the sole Submission destination, preserves non-`main` PRs for explicit human repair, and lets every valid pass reach Ready for Merge regardless of mergeability. Existing `sync` is tolerated as inert legacy state until ordinary implementation handoff removes it; old workers finish with the old CLI or stop for controlled cutover.

ADR 0005 supersedes the broad integration assignment above only for the bounded pre-Audit step: immediately before each submission Audit, the Agent Worker observes and merges one current `main` revision into the work branch and resolves permitted conflicts. The Workflow Engine still owns no target snapshot or merge operation, later target movement does not restart the round, and post-review integration plus final merge remain with the human Merge Authority.

The endpoint-validation slice supersedes consequence 13 only for markerless Artifact Baseline and Artifact Completion evidence: there is no lazy automatic Adoption. Existing workers stay on the former CLI until an explicit handoff supplies available full endpoint SHAs on each relevant invocation; those flags are transient, Work Item-scoped, and preserved through generated resume, inspection, and handoff commands.

The Review Checkpoint slice replaces bounce history with a worktree-private `<count>:<sha>` record. The CLI alone maintains it, counts completed reviews including Needs Human, and applies a default limit of two that restricts further automatic Rework but never a passing verdict. Resume instructions direct a fresh agent to inspect existing progress; the checkpoint is not an agent-facing artifact, and neither execution recovery nor review counting requires a hidden issue journal or PR timeline reconstruction.

The remaining planned slices make `next` select, claim, and render without local project objects and simplify transition recovery. Those changes are not implied by the delivered integration policy.
