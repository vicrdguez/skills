...
The [candidate-first correction](../capabilities/work-item-lifecycle.md#planned-correction-candidate-first-workflow) changes earlier compatibility and review policies while preserving agent judgment, independent Watchdog Review, and human-only merging. The delivered integration-policy slice removes implementation target pins, uses `main` as the sole Submission destination, preserves non-`main` PRs for explicit human repair, and lets every valid pass reach Ready for Merge regardless of mergeability. Existing `sync` is tolerated as inert legacy state until ordinary implementation handoff removes it; old workers finish with the old CLI or stop for controlled cutover.

The remaining planned slices make `next` select, claim, and render without local project objects, introduce explicit marker endpoints, replace bounce history with a worktree-private Review Checkpoint, and simplify transition recovery. Those changes are not implied by the delivered integration policy.
