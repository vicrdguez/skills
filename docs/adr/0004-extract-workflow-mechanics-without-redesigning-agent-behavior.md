...
The candidate-first correction changes earlier compatibility, synchronization, and bounce-counting policies while preserving agent judgment, independent Watchdog Review, and human-only merging. The delivered integration-policy slice removes implementation target pins, uses `main` as the sole Submission destination, preserves non-`main` PRs for explicit human repair, and lets every valid pass reach Ready for Merge regardless of mergeability. Existing `sync` is tolerated as inert legacy state until ordinary implementation handoff removes it; old workers finish with the old CLI or stop for controlled cutover.
