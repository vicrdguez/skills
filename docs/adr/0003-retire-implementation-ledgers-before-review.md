...
## Planned correction: Endpoint validation

The [candidate-first correction](../capabilities/work-item-lifecycle.md#planned-correction-candidate-first-workflow) replaces tree-transition snapshot discovery and intermediate-content enforcement with slice-scoped `[baseline] <slice-slug>` and `[completion] <slice-slug>` commit-subject markers. The accepted requirement is that endpoint paths, modes, and contents differ only by permitted completion ticks, with valid ancestry, completed automated checks, unchecked Manual Verification, and ledger absence at the review head; intermediate edits restored before completion are deliberately irrelevant. Completion still contains the ledger and removal follows it, while ambiguous matching markers are refused rather than guessed. Markerless work may use explicit endpoint SHA overrides per validation/handoff invocation, applying the same checks without rewriting commits, automatic fallback, or persisted adoption records. Workers obtain historical contents after Git preparation instead of requiring startup packets to embed them, preserving a fast selection path without local project-object prerequisites.
