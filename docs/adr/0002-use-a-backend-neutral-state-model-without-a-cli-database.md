...
- Multi-system handoffs publish exact evidence and finish source cleanup while the destination remains protected, then release its Claim last. Retries continue only when current projections, Git evidence, supplied Result Documents, and independently owned policy evidence prove the direction; no transition journal or rollback protocol is authoritative.
...
- Interrupted V1 work resumes by stable Work Item identity from its worktree or an explicit resume command; ordinary queue selection skips `wip` projections. Resume inspects preserved Git, artifacts, and visible feedback rather than persisted Agent Worker progress.
