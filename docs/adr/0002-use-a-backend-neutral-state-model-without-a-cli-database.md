...
- Multi-system handoffs publish exact evidence and finish source cleanup while the destination remains protected, then release its Claim last. Retries continue only when current projections, Git evidence, supplied Result Documents, and independent pins prove the direction; no transition journal or rollback protocol is authoritative.
