...
Implement `next` and `resume` also take `--mode standard|team` (default `standard`), and the opaque values `--helper-model`, `--helper-thinking`, `--reviewer-model` and `--reviewer-thinking`. Team mode replaces the Execution Skill's delegation guidance with the team strategy. A reviewer value renders in the Audit dispatch step and a helper value in the team delegation step; helper values need team mode. An unknown mode is refused before anything is claimed, and the bound resume command repeats the supplied values.
