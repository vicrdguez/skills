...
- `prose/craft/<name>.md` is a Craft file. `prose/craft/audit.md` holds the two review axes that both Audit Procedures include. `prose/craft/review.md` is the adversarial review method that Watchdog includes, together with Audit's acceptance criteria. `prose/craft/team.md` is the team strategy that Implement's delegation step includes in team mode.
...
- `prose/adapters/stub.md` is the stub template, and `prose/adapters/stubs/<name>.md` is each skill's discovery frontmatter. `prose/adapters/entry.md` is the Harness Adapter template for `implement`, `implement-team` and `watchdog`. `skl install` renders it into each harness's native entry point: a pi prompt template, a Claude Code skill only the user invokes, and an OpenCode command. Each gets the frontmatter keys its harness recognizes, and the Implement adapters pass their model and thinking arguments in the harness's placeholder syntax. Codex has no entry point that takes arguments, so it keeps the stubs.
