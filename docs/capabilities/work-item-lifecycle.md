...
- Counts completed Watchdog Reviews from a CLI-owned Review Checkpoint. The default limit of two sends a failure at or beyond the limit to Needs Human, while a pass remains eligible at any count.
- Stores `<count>:<full-reviewed-sha>` atomically in `.watchdog` inside the selected linked worktree's private absolute Git directory. Missing storage starts at zero; malformed or inaccessible storage is refused rather than reset. Nonterminal work and working-tree cleaning retain it.
- `skl watchdog next` claims the oldest unclaimed Awaiting Review Submission by creation time and Work Item identity. `resume --item <number>` refreshes the invocation's actual PR head without selecting another item or persisting an in-progress head.
- Supplies completed count, intended review number, reviewed head, and full or incremental scope. An unavailable or nonancestral retained head widens scope without discarding a valid count.
...
- `skl watchdog submit` accepts a semantic pass, rework, or Needs Human verdict with an explicit fixed `--review-number`. Structured anchors carry source path, line, side, and fixed commit; exact retries reuse published evidence, record the reviewed head before Claim release, and stop when the retained command cannot prove the intended round.
- A pass transports the complete agent-authored final PR body with Manual Verification and appends `Closes #N`, projecting Ready for Merge through `done` without closing the source issue. Permitted Debt Marker comments require a pushed final head distinct from the actual reviewed head and a worker-owned Post-Marker Check, not CLI source parsing. Verified `done` removes the checkpoint; local cleanup failure is a success warning, not a reason to republish.
- Removing and recreating a dedicated worktree intentionally resets its review budget. Counts are not rebuilt from GitHub timelines, summaries, shared registries, or backups.
- A conflicting pass pins the current target for Synchronization Rework, represented by `rework` plus `sync`; implementation removes `sync` when it hands back to review. The completed review remains recorded independently of this target policy.
