...
func InspectLedger(root, ref, slug string) (LedgerHistory, error) {
	...
	result.Baseline = resolveMarker(&result, slug, "baseline", markers["baseline"])
	result.Completion = resolveMarker(&result, slug, "completion", markers["completion"])
	if result.Baseline == "" {
...
func resolveMarker(result *LedgerHistory, slug, kind string, markers []string) string {
	if len(markers) > 1 {
