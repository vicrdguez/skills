	...
	for page := 1; ; page++ {
		...
		for _, comment := range batch {
			observed := skilldist.ReviewComment{
				Source: b.repositoryPath(repository) + stream, Body: comment.Body, Author: comment.User.Login, Association: comment.Association, Commit: comment.Commit, Path: comment.Path, CreatedAt: comment.CreatedAt, Side: comment.Side,
