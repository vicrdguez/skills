...
func (b *GitHubBackend) CoordinationItems(ctx context.Context, repository workflow.RepositoryID) ([]workflow.CoordinationItem, error) {
	...
	for _, issue := range issues {
		...
		parent := workflow.CoordinationItem{Number: issue.Number, Title: issue.Title, Closed: issue.State == "closed"}
		for page := 1; ; page++ {
...
func (b *GitHubBackend) CloseCoordination(ctx context.Context, repository workflow.RepositoryID, number int) error {
