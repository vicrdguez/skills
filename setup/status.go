...
func (b *GitHubBackend) CoordinationItems(ctx context.Context, repository github.RepositoryID) ([]workflow.CoordinationStatus, error) {
	issues, err := b.listIssues(ctx, repository)
	if err != nil {
		return nil, err
	}
	var parents []workflow.CoordinationStatus
	for _, issue := range issues {
		if len(issue.PullRequest) != 0 || issue.SubIssuesSummary.Total == 0 {
			continue
		}
		parent := workflow.CoordinationStatus{Number: issue.Number, Closed: issue.State == "closed"}
		for page := 1; ; page++ {
			...
			for _, child := range children {
				parent.Children = append(parent.Children, child.Number)
			}
			if len(children) < 100 {
				break
			}
...
func (b *GitHubBackend) CloseCoordination(ctx context.Context, repository github.RepositoryID, number int) error {
