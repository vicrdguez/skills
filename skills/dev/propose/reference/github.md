...
- **Child issues**: issue linked to the map as a GitHub sub-issue (`gh api` on the sub-issues endpoint).
...
Used by `/implement` and `/review`. Claims issues labeled as `ready`, `rework` or `review` by adding `wip`

As an implementor:
- Always prefer `rework` PRs than `ready` issues.
- Fetch the oldest open issue labeled as `ready` or `rework` and without `wip`.
...
As a reviewer
- Fetch the oldest open issue labeled as `review` and without `wip`.
...
**IMPORTANT**: Add the `wip` label without removing the lifecycle lable. Do not fetch or touch the change unless the command succeeds:
...
Used by `/review`. Subit the review, as either needing `rework` or as `done`
...
Reviewr findings go on the PR as comments. Inline comments anchor to a line; a summary comment carries the verdict.
