...
This continuation recovers the human-facing progress pull request presentation recorded for the latest local delivery result. It is presentation recovery, not implementation or review: the committed local result is already authoritative.
...
{{end}}
{{if eq .Condition "satisfied"}}The selected pull presentation already matches the latest recorded result. Author nothing, change nothing, and report this result.
...
{{else if eq .Condition "stale"}}The only available temporary body describes a superseded result. Do not publish it: author a fresh body for the current recorded result through the deferred resource below.
{{else if eq .Condition "prose-needed"}}No applicable temporary public body remains. Author a fresh descriptive progress body through the deferred resource below; never try to recover identical lost bytes.
{{else if .View.BodyPath}}A registered temporary body still matches this result at `{{.View.BodyPath}}`. Reuse that prose without editing it.
{{else}}No reusable temporary body is registered for this result. Author a fresh descriptive body through the deferred resource below.
{{end}}
...
{{if .View.Source.Reviewed}}Reviewed source head: `{{.View.Source.Reviewed}}`
{{end}}{{if .View.Source.Head}}Final source head: `{{.View.Source.Head}}`
...
- Do not rerun implementation, tests, Audit, Watchdog review, or acceptance. The recorded result is authoritative.
