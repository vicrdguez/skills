...
{{if .PauseCommand}}Pause with `{{.PauseCommand}}` for a permitted human decision when a consequential unresolved decision blocks progress; its typed outcome is `needs_human` and it may carry the source head and target flags when source progress exists. A pause does not approve, merge, retire `.changes`, or claim completion.
