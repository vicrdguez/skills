	...
	for _, stage := range []string{"entry", "transition", "later-guard"} {
		for _, mergeability := range []string{"unknown", "conflicting"} {
			...
			t.Run(stage+"/"+mergeability, func(t *testing.T) {
				...
				b := &statusGuardMemory{implementationMemory: &implementationMemory{work: []workflow.ImplementationItem{{Number: 7, Branch: "widget", State: workflow.ReadyForMerge, Claimed: true, Submission: &workflow.Submission{Number: 11, Head: head, ReviewedHead: head, Base: "main", State: workflow.ReadyForMerge, Claimed: true, PendingReview: workflow.ReadyForMerge, Mergeability: "mergeable", Bounces: 1}}}}}
...
func TestStatusRoutesAcceptedConflictToSynchronizationRework(t *testing.T) {
	...
	for _, pending := range []workflow.State{"", workflow.ReadyForMerge} {
		b := &implementationMemory{work: []workflow.ImplementationItem{{Number: 7, Branch: "widget", State: workflow.ReadyForMerge, Claimed: pending != "", Submission: &workflow.Submission{Number: 11, Head: "fixed", Base: "main", Mergeability: "conflicting", Bounces: 1, PendingReview: pending, Claimed: pending != ""}}}, remoteHeads: map[string]string{"main": target}}
