...
func TestWatchdogRecoversHandoffFromOriginalSummaryDocuments(t *testing.T) {
	for _, fault := range []string{"interrupted claim release", "receipt publication"} {
		t.Run(fault, func(t *testing.T) {
			...
			summaries, metadata, events := []map[string]any{}, []map[string]any{}, []map[string]any{}
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case p == "/pulls/11" || p == "/issues/11":
					result = pull
				case p == "/issues/7/comments" || p == "/issues/11/comments":
					stream := &metadata
					if p == "/issues/11/comments" {
						stream = &summaries
					}
					if r.Method == http.MethodPost {
						...
						if p == "/issues/7/comments" && failReceipt && strings.Contains(comment["body"].(string), `"outcome"`) {
				...
				case p == "/pulls/11/comments" || p == "/pulls/11/reviews":
				default:
			...
			submit := func() (setup.ImplementationOutput, error) {
				...
				if err := app.Run([]string{"skl", "watchdog", "submit", "--repo", root, "--remote", "origin", "--item", "7", "--reviewed-head", head, "--head", head, "--verdict", "needs-human", "--summary", summary}); err != nil {
