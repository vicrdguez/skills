...
func TestW5PacketUsesSubmissionIdentityNotFeedback(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, guidance := range []string{"This invocation follows finding-driven Rework:", "This obligation holds even when the supplied feedback is empty or still pending"} {
