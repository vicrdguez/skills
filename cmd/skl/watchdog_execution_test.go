...
func TestWatchdogNextDeliversCompleteFirstReviewInMarkdown(t *testing.T) {
	...
	got := output.String()
	for _, want := range []string{"name: watchdog", "Work Item #7", "Submission is #11", head, "Full Gate", "Artifact Baseline", "Audit ledger", "critical", "skl watchdog submit", "skl skill --resource reference/acceptance.md audit", "skl skill --resource reference/review.md", "every accepted obligation", "specific evidence gap", "Post-Marker Check", "Manual verification", "--head <actual-pushed-final-SHA>", "human"} {
