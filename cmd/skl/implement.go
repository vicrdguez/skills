...
func implementationCommands(newBackend backendFactory, stdout io.Writer) []*cli.Command {
	...
	for _, name := range []string{"next", "resume", "inspect", "submit", "needs-human"} {
		commands = append(commands, &cli.Command{Name: name,
			Flags: []cli.Flag{&cli.PathFlag{Name: "repo", Value: "."}, &cli.StringFlag{Name: "remote"}, &cli.IntFlag{Name: "item"}, &cli.PathFlag{Name: "body"}, &cli.PathFlag{Name: "decision"}, &cli.StringFlag{Name: "reason"}, &cli.StringFlag{Name: "reviewed-head"}},
			Action: func(command *cli.Context) error {
				...
				if name == "inspect" {
					...
					outcome, err = workflow.StartImplementation(command.Context, command.Path("repo"), command.String("remote"), id, command.String("reviewed-head"), port)
				}
				if err != nil {
